...
class GridFieldHasOneRelationHandler extends GridFieldRelationHandler
{
    ...
    public function __construct(DataObject $onObject, $relationName, $targetFragment = 'before')
    {
        ...
        $objectHasOneRelationships = $onObject->hasOne();

        if (!isset($objectHasOneRelationships[$relationName])) {
            user_error(
                'Unable to find a has_one relation named ' . $relationName . ' on ' . $onObject->ClassName,
                E_USER_WARNING
            );
        }
        $this->targetObject = $onObject->$relationName;
    ...
    public function getColumnContent($gridField, $record, $columnName)
    {
        ...
        if (!($class == $this->targetObject || is_subclass_of($class, $this->targetObject))) {
            user_error(
                $class . ' is not a subclass of ' . $this->targetObject . '. ' .
                    'Perhaps you wanted to use ' . $this->targetObject . '::get() as the list for this GridField?',
                E_USER_WARNING
            );
        }
        ...
        return $field->renderWith('Firesphere/GridFieldRelationHandler/GridFieldHasOneRelationHandlerItem');
    }
